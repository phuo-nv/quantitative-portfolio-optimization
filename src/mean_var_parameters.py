...
# Type alias for weight bounds (per-asset array, dict by name, or uniform float)
WeightBound = Union[np.ndarray, dict[str, float], float, None]


@dataclass
class MeanVarParameters:
    """
    ...
    # Weight / cash bounds
    w_min: WeightBound = 0.0  # Lower bound for each risky weight
    w_max: WeightBound = 1.0  # Upper bound for each risky weight
    ...
    def update_w_min(self, new_w_min: WeightBound) -> None:
        """Update the lower bound for risky weights."""
        self.w_min = new_w_min

    def update_w_max(self, new_w_max: WeightBound) -> None:
